...
def get_dataset(img_size):
    ...
    with open("/home/akhilkumar/Desktop/AttrLostGAN/datasets/vg/vocab.json", "r") as read_file:
        vocab = json.load(read_file)

    data = VgSceneGraphDataset(vocab=vocab, h5_path='/home/akhilkumar/Desktop/AttrLostGAN/datasets/vg/train.h5',
                               image_dir='/home/akhilkumar/Desktop/AttrLostGAN/datasets/vg/images/',
...
def main(args):
    ...
    #    torch.load(netG, os.path.join(args.out_path, 'model/', 'Ge_%d.pth' % (epoch + 1)))
    #    torch.load(netD, os.path.join(args.out_path, 'model/', 'De_%d.pth' % (epoch + 1)))
...
#    commit_obj = get_recent_commit()
 ...
 #   logger.info('Commit Tag: ' + commit_obj)
    ...
    netG = torch.load(os.path.join(args.out_path, 'model/', 'Ge_%d.pth' % (epoch + 1)))
    netG.cuda()
    netG.eval()
    netD = torch.load(os.path.join(args.out_path, 'model/', 'De_%d.pth' % (epoch + 1)))
    netD.cuda()
    netD.eval()
    
    i = epoch + 1
    for epoch in range(i, args.total_epoch):
        ...
        torch.save(netG, os.path.join(args.out_path, 'model/', 'Ge_%d.pth' % (epoch + 1)))
        torch.save(netD, os.path.join(args.out_path, 'model/', 'De_%d.pth' % (epoch + 1)))
