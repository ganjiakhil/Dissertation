...
from model.mask_regression import MaskRegressNetv2
from model.norm_module import SpatialAdaptiveSynBatchNorm2dv2
...
class ResnetGenerator64(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3, num_attrs=None):
        super(ResnetGenerator64, self).__init__()
        self.num_classes = num_classes
        self.num_attrs = num_attrs

        self.label_embedding = nn.Embedding(num_classes, 180, padding_idx=0)
        self.attribute_features = nn.Sequential(nn.Linear(num_attrs, 2048),
                                                nn.ReLU(),
                                                nn.Linear(2048, 1024),
                                                nn.ReLU(),
                                                nn.Linear(1024, 512),
    ...
    def forward(self, z, bbox, z_im=None, y=None, attrs=None):
        b, o = z.size(0), z.size(1)
        label_embedding = self.label_embedding(y)

        z = z.view(b * o, -1)
        label_embedding = label_embedding.view(b * o, -1)
        attr_feat = self.attribute_features(attrs.float())
        attr_feat = attr_feat.view(b * o, -1)

        latent_vector = torch.cat((z, label_embedding, attr_feat), dim=1).view(b, o, -1)
        ...
        bbox_mask_ = bbox_mask(z, bbox, 32, 32)
...
class ResnetGenerator256(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3, num_attrs=None):
        ...
        self.res5 = ResBlock(ch * 4, ch * 2, upsample=True, num_w=num_w, psp_module=False)
...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, h_ch=None, ksize=3, pad=1, upsample=False, num_w=128, predict_mask=True,
                 psp_module=False):
        ...
        self.b1 = SpatialAdaptiveSynBatchNorm2dv2(in_ch, num_w=num_w, batchnorm_func=BatchNorm)
        self.b2 = SpatialAdaptiveSynBatchNorm2dv2(self.h_ch, num_w=num_w, batchnorm_func=BatchNorm)
...
def bbox_mask(x, bbox, H, W):
    ...
    X = torch.linspace(0, 1, steps=W).view(1, W).expand(N, W).to(device=x.device)
    Y = torch.linspace(0, 1, steps=H).view(1, H).expand(N, H).to(device=x.device)
